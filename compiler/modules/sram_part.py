...
class sram_1bank(design, verilog, lef):
    ...
    def add_pin_col_addr_dff(self):
        ...
        for port in self.all_ports:
            for bit in range(self.col_addr_size):
                ...
                #output
                self.add_pin("a{}_{}".format(port, bit), "OUTPUT")
            #clk_buf, regard as input
            self.add_pin("clk_buf{}".format(port), "INPUT")
            # Standard supply and ground names
            try:
    ...
    def add_pin_data_dff(self):
        ...
        for port in self.all_ports:
            for bit in range(self.word_size + self.num_spare_cols):
                ...
                # output
                self.add_pin("bank_din{}_{}".format(port, bit), "OUTPUT")
            #clk_buf, regard as input
            self.add_pin("clk_buf{}".format(port), "INPUT")
            # Standard supply and ground names
            try:
    ...
    def add_pin_wmask_dff(self):
        ...
        for port in self.all_ports:
            for bit in range(self.num_wmasks):
                ...
                # output
                self.add_pin("bank_wmask{}_{}".format(port, bit), "OUTPUT")
            #clk_buf, regard as input
            self.add_pin("clk_buf{}".format(port), "INPUT")
            # Standard supply and ground names
            try:
    ...
    def add_pin_spare_wen_dff(self):
        ...
        for port in self.all_ports:
            for bit in range(self.num_spare_cols):
                ...
                # output
                self.add_pin("bank_spare_wen{}_{}".format(port, bit), "OUTPUT")
            #clk_buf, regard as input
            self.add_pin("clk_buf{}".format(port), "INPUT")
            # Standard supply and ground names
            try:
    ...
    def route_bank_only(self):
        ...
        pins_to_route_bank = self.add_pins_to_route_bank()
        print("====================================================")
    ...
    def route_control_only(self, instance_index=0):
        ...
        pins_to_route_control = self.add_pins_to_route_control(instance_index=instance_index)
        print("====================================================")
        for pin in self.pins:
            print(pin)
        ...
        if OPTS.perimeter_pins:
            ...
            rtr = router(layers=self.m3_stack,
                         bbox=init_bbox,
                         design=self)
            rtr.route(pins_to_route_control)
        if OPTS.route_supplies:
            self.route_supplies(init_bbox)
    ...
    def route_row_addr_dff_only(self, instance_index=0):
        ...
        pins_to_route_row_addr_dff = self.add_pins_to_route_row_addr_dff(instance_index=instance_index)
        print("====================================================")
        for pin in self.pins:
            print(pin)
        ...
        if OPTS.perimeter_pins:
            ...
            rtr = router(layers=self.m3_stack,
                         bbox=init_bbox,
                         design=self)
            rtr.route(pins_to_route_row_addr_dff)
        if OPTS.route_supplies:
            self.route_supplies(init_bbox)
    ...
    def route_col_addr_dff_only(self, instance_index=0):
        ...
        pins_to_route_col_addr_dff = self.add_pins_to_route_col_addr_dff(instance_index=instance_index)
        print("====================================================")
        for pin in self.pins:
            print(pin)
        ...
        if OPTS.perimeter_pins:
            ...
            rtr = router(layers=self.m3_stack,
                         bbox=init_bbox,
                         design=self)
            rtr.route(pins_to_route_col_addr_dff)
        if OPTS.route_supplies:
            self.route_supplies(init_bbox)
    ...
    def route_data_dff_only(self, instance_index=0):
        ...
        pins_to_route_data_dff = self.add_pins_to_route_data_dff(instance_index=instance_index)
        print("====================================================")
        for pin in self.pins:
            print(pin)
        ...
        if OPTS.perimeter_pins:
            ...
            rtr = router(layers=self.m3_stack,
                         bbox=init_bbox,
                         design=self)
            rtr.route(pins_to_route_data_dff)
        if OPTS.route_supplies:
            self.route_supplies(init_bbox)
    ...
    def route_wmask_dff_only(self, instance_index=0):
        ...
        pins_to_route_wmask_dff = self.add_pins_to_route_wmask_dff(instance_index=instance_index)
        print("====================================================")
        for pin in self.pins:
            print(pin)
        ...
        if OPTS.perimeter_pins:
            ...
            rtr = router(layers=self.m3_stack,
                         bbox=init_bbox,
                         design=self)
            rtr.route(pins_to_route_wmask_dff)
        if OPTS.route_supplies:
            self.route_supplies(init_bbox)
    ...
    def route_spare_wen_dff_only(self, instance_index=0):
        ...
        pins_to_route_spare_wen_dff = self.add_pins_to_route_spare_wen_dff(instance_index=instance_index)
        print("====================================================")
        for pin in self.pins:
            print(pin)
        ...
        if OPTS.perimeter_pins:
            ...
            rtr = router(layers=self.m3_stack,
                         bbox=init_bbox,
                         design=self)
            rtr.route(pins_to_route_spare_wen_dff)
        if OPTS.route_supplies:
            self.route_supplies(init_bbox)  
    ...
    def add_pins_to_route_spare_wen_dff(self, instance_index=0):
        ...
        for bit in range(self.num_spare_cols):
            ...
            # output
            pins_to_route.append("bank_spare_wen{}_{}".format(port, bit))
        #clk_buf, regard as input
    ...
    def add_pins_to_route_wmask_dff(self, instance_index=0):
        #only contains signal pins, no power pins
        """ Adding to route pins for wmask addr dff """
        pins_to_route = []
        port = instance_index  
        for bit in range(self.num_wmasks):
            # input
            pins_to_route.append("wmask{}[{}]".format(port, bit))
            # output
            pins_to_route.append("bank_wmask{}_{}".format(port, bit))
        #clk_buf, regard as input
        pins_to_route.append("clk_buf{}".format(port))
    ...
    def add_pins_to_route_data_dff(self, instance_index=0):
        #only contains signal pins, no power pins
        """ Adding to route pins for col addr dff """
        pins_to_route = []
        port = instance_index         
        for bit in range(self.word_size + self.num_spare_cols):
            # input
            pins_to_route.append("din{}[{}]".format(port, bit))
            # output
            pins_to_route.append("bank_din{}_{}".format(port, bit))
        #clk_buf, regard as input
        pins_to_route.append("clk_buf{}".format(port))
    ...
    def add_pins_to_route_col_addr_dff(self, instance_index=0):
        #only contains signal pins, no power pins
        """ Adding to route pins for col addr dff """
        pins_to_route = []
        port = instance_index 
        for bit in range(self.col_addr_size):
            #input
            pins_to_route.append("addr{}[{}]".format(port, bit))
            #output
            pins_to_route.append("a{}_{}".format(port, bit))
        #clk_buf, regard as input
        pins_to_route.append("clk_buf{}".format(port))  
    ...
    def add_pins_to_route_row_addr_dff(self, instance_index=0):
        #only contains signal pins, no power pins
        """ Adding to route pins for row addr dff """
        pins_to_route = []
        port = instance_index 
        for bit in range(self.row_addr_size):
            #input
            pins_to_route.append("addr{}[{}]".format(port, bit + self.col_addr_size))
            #output
            pins_to_route.append("a{}_{}".format(port, bit + self.col_addr_size))
        #clk_buf, regard as input
        pins_to_route.append("clk_buf{}".format(port))
    ...
    def add_pins_to_route_control(self, instance_index=0):
        #only contains signal pins, no power pins
        """ Adding to route pins for control """
        pins_to_route = []
        port = instance_index
        # Inputs
        pins_to_route.append("csb{}".format(port))
        if port in self.readwrite_ports:
            pins_to_route.append("web{}".format(port)), 
        pins_to_route.append("clk{}".format(port))
        if self.has_rbl:
            pins_to_route.append("rbl_bl{}".format(port))
        # Outputs
        if port in self.read_ports:
            pins_to_route.append("s_en{}".format(port)) 
        if port in self.write_ports:
            pins_to_route.append("w_en{}".format(port)) 
        pins_to_route.append("p_en_bar{}".format(port)) 
        pins_to_route.append("wl_en{}".format(port)) 
        pins_to_route.append("clk_buf{}".format(port))
    ...
    def add_pins_to_route_bank(self):
        #only contains signal pins, no power pins
        """ Adding to route pins for Bank """
        pins_to_route = []
        for port in self.read_ports:
            for bit in range(self.word_size + self.num_spare_cols):
        ...
        for port in self.all_ports:
            if self.has_rbl:
                pins_to_route.append("rbl_bl{0}".format(port))
        for port in self.write_ports:
            for bit in range(self.word_size + self.num_spare_cols):
                pins_to_route.append("bank_din{0}_{1}".format(port, bit))
        for port in self.all_ports:
            for bit in range(self.bank_addr_size):
                pins_to_route.append("addr{0}_{1}".format(port, bit))

        # For more than one bank, we have a bank select and name
        # the signals gated_*.
        for port in self.read_ports:
            pins_to_route.append("s_en{0}".format(port))
        for port in self.all_ports:
            pins_to_route.append("p_en_bar{0}".format(port))
        for port in self.write_ports:
            pins_to_route.append("w_en{0}".format(port))
            for bit in range(self.num_wmasks):
                pins_to_route.append("bank_wmask{0}_{1}".format(port, bit))
            for bit in range(self.num_spare_cols):
                pins_to_route.append("bank_spare_wen{0}_{1}".format(port, bit))
        for port in self.all_ports:
        ...
        for port in self.write_ports:
            for bit in range(self.word_size + self.num_spare_cols):
                self.add_io_pin(self.bank_inst,
                                ...
                                "bank_din{0}_{1}".format(port, bit),
                                start_layer=pin_layer) 
